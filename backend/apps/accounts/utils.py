...
from sentence_transformers import SentenceTransformer
from .models import Pulse, Alert, UrgentRequest
...
def ask_ai(user_question, search_context="all"):
    """
    Takes a user question, searches the relevant Postgres models via vector similarity,
    and returns a natural language answer generated by the LLM.
    """
    # 1. Convert the user's question into a 768-dim vector
    question_vector = embedding_model.encode(user_question).tolist()

    context_text = "Database Records:\n\n"

    # 2. Search Pulses (Services/Items)
    if search_context in ["all", "pulses"]:
        ...
        ).order_by('distance')[:3]  # Get top 3 closest matches

        for p in similar_pulses:
            context_text += f"- PULSE: {p.title} | Desc: {p.description} | Price: {p.price} {p.currencyType}\n"
    ...
    if search_context in ["all", "alerts"]:
        ...
        for a in similar_alerts:
            context_text += f"- ALERT: {a.title} | Category: {a.get_category_display()} | Details: {a.description}\n"

    # 4. Search Urgent Requests
    if search_context in ["all", "requests"]:
        ...
        for r in similar_requests:
            context_text += f"- URGENT REQUEST: {r.title} | Desc: {r.description} | Max Price: {r.max_price}\n"

    # 5. Send the retrieved data to the AI
    ...
    Guidelines:
    1. Provide a natural, friendly, and accurate response based only on the provided context.
    2. Do NOT mention "database", "records", or the source of the data to the user.
    3. If the context does not contain the answer, politely say: "I couldn't find any listings or information about that on the platform."
